...
doc_content_list=[]
for t,k in zip(corpus, corpus1):
    doc_content_list.append((del_http_user_tokenize(t), del_http_user_tokenize(k)))
max_len_seq=0
max_len_seq_idx=-1
min_len_seq=1000
min_len_seq_idx=-1
...
for i,seq in enumerate(doc_content_list):
    ...
    if len(seq)>max_len_seq:
        max_len_seq=len(seq)
        max_len_seq_idx=i
    if len(seq1)<min_len_seq:
        min_len_seq=len(seq1)
        min_len_seq_idx=i
    if len(seq1)>max_len_seq:
        max_len_seq=len(seq1)
        max_len_seq_idx=i
...
for doc_content in doc_content_list:
    new_doc = (clean_str(doc_content[0]), clean_str(doc_content[1])
    if cfg_use_bert_tokenizer_at_clean:
        sub_words = bert_tokenizer.tokenize(new_doc[0])
        sub_words1 = bert_tokenizer.tokenize(new_doc[1])
        sub_doc=' '.join(sub_words).strip()
        sub_doc1=' '.join(sub_words1).strip()
        new_doc=(sub_doc, sub_doc1)
    new_doc_content_list.append(new_doc)
    for word in new_doc[0].split():
        if word in tmp_word_freq:
            tmp_word_freq[word] += 1
        else:
            tmp_word_freq[word] = 1
    for word in new_doc[1].split():
        if word in tmp_word_freq:
            tmp_word_freq[word] += 1
        else:
            tmp_word_freq[word] = 1
...
for i,doc_content in enumerate(doc_content_list):
    words = doc_content[0].split()
    words1 = doc_content[1].split()
    doc_words = []
    for word in words:
        if cfg_ds in ('mr','sst','cola'):
    ...
    if doc_str == '':
        count_void_doc+=1
        print('No.',i, 'is a empty doc after treat, replaced by \'%s\'. original:%s'%(doc_str,doc_content[0]))
    doc_words1 = []
    for word in words1:
        if cfg_ds in ('mr','sst','cola'):
            doc_words1.append(word)
        elif word not in stop_words and tmp_word_freq[word] >= freq_min_for_word_choice:
            doc_words1.append(word)
    doc_str1 = ' '.join(doc_words1).strip()
    if doc_str1 == '':
        count_void_doc+=1
        print('No.',i, 'is a empty doc after treat, replaced by \'%s\'. original:%s'%(doc_str1,doc_content[1]))
    clean_docs.append((doc_str, doc_str1))
...
for i,line in enumerate(clean_docs):
    temp = line[0].strip().split() + line[1].strip().split()
...
for doc_words in shuffled_clean_docs:
    words = doc_words[0].split() + doc_words[1].split()
    for word in words:
        word_set.add(word)
...
for i in range(len(for_idf_docs)):
    doc_words = for_idf_docs[i][0] + for_idf_docs[i][1] 
...
for doc_id in range(n_docs):
    doc_words = shuffled_clean_docs[doc_id][0] + shuffled_clean_docs[doc_id][1]
    words = doc_words.split()
    for word in words:
        try:
            word_id = vocab_map[word.replace("?", "")]
...
for i in range(n_docs):
    doc_words = shuffled_clean_docs[i][0] + shuffled_clean_docs[i][1]
    words = doc_words.split()
    doc_word_set = set()
    tfidf_vec = []
    for word in words:
